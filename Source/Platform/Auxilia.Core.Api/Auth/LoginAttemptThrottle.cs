...
/// <summary>
/// Per-username failed-attempt throttle for <c>POST /auth/login</c>: after
/// <see cref="CoreSecuritySettings.LoginFailureLimitPerUsername"/> failures within the sliding
/// window, further attempts for that username are refused until the oldest failure ages out.
/// Complements the per-IP rate-limit policy — this one follows the ACCOUNT, so guessing that
/// rotates source IPs is still throttled. A successful sign-in clears the username's slate.
/// </summary>
