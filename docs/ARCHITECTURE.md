...
- **Run API**: start a run inline ("on the fly"), from a stored configuration, or from a startup static seed; query runs with typed filters; cancel a run. It authorizes the caller, then dispatches a **self-contained `RunWorkflowCommand`** to Core.Runner over the bus (the runner has a different database and cannot resolve a Core principal, so it trusts Core-dispatched commands rather than re-authorizing). The command also carries the registry's inspected **schema** as a cold-start seed: a runner that has never run the type makes its schema-driven pre-launch decisions (interactive terminal, network baseline, declared repositories) correctly on the very first dispatch; the runner's own store — refreshed by each run's registration — wins once populated.
...
- **The resolution token is never stored in the clear**: the Core keeps only its SHA-256 digest plus a redacted dispatch-command copy (a failover/rerun re-dispatch mints a fresh token and package URL) — enough to verify what the runner presents, never to replay it — and the runner protects its own persisted copy with its settings protector
