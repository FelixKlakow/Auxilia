...
- **Reaps orphaned workflow containers at startup** (label `auxilia.workflow=1`, label-scoped — nothing else on the Docker host is touched); pairs with the Core's zombie sweep: the FailoverMonitor also fails over non-terminal runs whose owner was never heard from once their record goes stale, and a failover **re-dispatch goes through the same machinery as an operator rerun** (fresh command id + resolution token, bindings re-stashed).
