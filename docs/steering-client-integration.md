...
- **Detail-tag contract (generic, 2026-07-28):** the WORKFLOW tags additional chat entries via `AgentChatEntry.DetailTag` (open vocabulary, e.g. `permissions`, `bookkeeping`) and thereby declares them DEFAULT-HIDDEN; clients render one generic per-tag show toggle from the tags they see and never hardcode tag semantics. Currently tagged: permission narration (both agents) and plan/task bookkeeping tool calls (`TaskCreate`/`TaskUpdate`/`TodoWrite`/`ExitPlanMode` — whose approved plan also feeds the plan view as a checklist snapshot).
...
- **Session vocabulary (out):** `{"$type":"session-vocabulary","models":[{id,label,efforts,defaultEffort}]}` — the agent announces its LIVE model/reasoning vocabulary (Copilot: SDK `ListModelsAsync`); steering client dropdowns feed from it, never from static lists. New session setting `reasoning-effort` rides `SetModelAsync` (Copilot); Claude sessions announce no efforts, so the control hides. Model lists for runs without a vocabulary come from the provider's registered `ProviderModelCatalog` (data-driven browse kind `models` — the Core executes registration data, no vendor logic).
