...
- **P3 OPEN**: Docker system test needs a DRIVEN-capable stub CLI (honors tmux input, posts fake hook events, writes exchange files) — the plain stubs are headless-only; markdown-rendered plan/bundle viewers in the steering client (v1 uses the monospace Detail block, which works); Copilot console EVENTS (CLI has no hooks — session-log tail investigation); first REAL end-to-end run against a live AzDO story (needs Felix's tenant).
