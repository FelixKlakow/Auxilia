...
- **Session terminal** — the old ttyd reverse-proxy (`SessionTerminalProxy`) has no Core equivalent; dropped from the console. Now MORE urgent (2026-07-28): `claude-code` gained `view-mode=console` (the CLI in tmux+ttyd, terminal exposed per-run via the new schema-declared `InteractiveTerminalGate`) — the container side is done and `TerminalEndpoint` is stamped on the run record, but without an authenticated Core.Api proxy no client can reach it. Follow-ups: the proxy endpoint (+ `ICoreClient` surface + a terminal pane in the steering client/AdminConsole for runs with a `TerminalEndpoint`), and a console-mode Docker system test (stub CLI under tmux).
