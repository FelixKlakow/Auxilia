...
- **P3 mostly delivered 2026-07-28 late**: driven-capable stub (`driven-stub.sh`) + full-pipeline Docker system test WRITTEN (ImplementationWorkflowSystemTests; git server + email work-items provisioning in EndToEndEnvironment) — compiles, stub verified; the FIRST live run must still go green (one attempt timed out at 5 min under a heavily loaded Docker host — diagnose fresh). Base prompts per role (`base-prompt`, `author-base-prompt`/`reviewer-base-prompt`; Claude `--append-system-prompt`, others first-prompt), markdown gate details + MERMAID diagrams (AgentView.Wpf FenceRenderer + hub WebView2 renderer), `Browse: "models"` on both CLI Model settings (live model dropdown at setup), run/config forms grouped (defaulted inputs fold into "Advanced"), and the clickable step flow (`WorkflowStepFlow` "flow" view → steering client chips) all landed. NOTE: after schema-affecting input changes, RE-REGISTER types (`--emit-schema` → POST /api/workflow-types + approve) or the steering client shows stale inputs. Remaining: Copilot console EVENTS (CLI has no hooks; CONFIRMED it has `--log-dir`/`--log-level` — run one real authed console session, inspect the log shape, then build the tail-based `IConsoleSessionEventSource`); first REAL end-to-end run against a live AzDO story (needs Felix's tenant). Markdown-rendered gate details DELIVERED 2026-07-28 (`OperatorQuestion.DetailFormat` "markdown" → AgentView.Wpf MarkdownViewer in the decision cards; permission tool inputs stay monospace).

- **Declared flow (presentation metadata) DELIVERED 2026-07-29**: `DeclaresFlow`/`FlowStepDescriptor(Id, Label, Description, SkipInput, SkipValue)` in `WorkflowSchema.Flow`+`WorkflowManifest.Flow`+`WorkflowSchemaDto.Flow`; the runtime `flow` view slimmed to per-step states (`WorkflowStepState`), joined client-side. the steering client renders the stage view in the CONFIGURE and RUN panels (skip hints dim live with the toggles) and the monitor joins live states (shared `FlowStrip` control). Dev stack scripts added: `Start-DevStack.ps1` (the launch recipe, previously session-memory only) + `Cleanup-TestWorkflows.ps1` (testing configurations/types/finished runs — run after test sessions so the steering client stays clean; executed 2026-07-29, echo/sleeping/crashing/steering-sample family removed from the dev Core). Direction: more presentation metadata (input grouping/ordering) can follow the same schema-declared pattern instead of per-workflow frontend code. OPEN: workflow images (implementation/claude-code/copilot) need a rebuild for the new flow shape — blocked 2026-07-29 by a router-DNS failure resolving mcr.microsoft.com; retry `docker build` when DNS recovers, else runs pre-flight-fail on assembly skew.
