...
> **Status:** Draft / design exploration · 2026-07-25 · **no implementation committed yet.**
> **Owner:** Felix Klakow
> **Target codebase:** the **the steering client** product — a *separate, current* codebase. **This repository (Auxilia) is a reference/predecessor, not the target**; it is cited only for proven primitives to reuse or adapt. The the steering client's repository location is supplied by Felix out-of-band (ask for the path if it isn't given) — it is **not** in this repo.
> **Reading note (for a fresh session):** this document is self-contained — you do **not** need the originating chat. It supersedes an earlier *"Human Approval Queues"* draft. Two things changed in review: (1) *approval queues → a steering **spectrum***, and (2) *the mechanism does **not** belong in the dumb Core — steering semantics live in the product layer*. The reference code is embedded in Appendix A (the original lived in a session-temp scratchpad that will not survive).
