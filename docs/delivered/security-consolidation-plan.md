...
> **Status:** Approved · 2026-07-25 · continues `docs/delivered/core-platform-separation-plan.md` (Phase 2's remaining intent)
> **Owner:** Felix Klakow
...
>
> **Delivery status (2026-07-25):** **S1 ✅** (`6f31c2b`), **S2 ✅** (`3f486f4`), dispatcher decoupled (`7683f22`), and the **legacy removal is complete** (`609e547`): `SlotActivationHandler` is Core-only, and the runner's entire slot-config subsystem is deleted — `SlotConfigurationStore`, `WorkflowConfigurationStore`, `SlotInstanceStore`, `SlotConfigurationSeedHandler` + the `slot-configurations` fanout, `SlotConfigurationsSettings`, `DirtyConfigurationDetector`, `ConfigurationResolver` — with `WorkflowRegistrationHandler` rewired (persists the schema, resolves signal handlers, no slot-config validation). **The runner database now holds no connector secrets — "secrets only in the Core" is enforced, not just achievable.** All green (solution-wide unit + component). **Remaining:**
> - **Docker acceptance ✅** — the `credential-resolution-workflow` dummy declares a slot bound to a Core **connector** (holding a random secret); on real Docker the run resolves the credential **JIT through the Core**, the runner relays the ciphertext, and the workflow decrypts + verifies the secret (which exists only in the connector, never in the image). Backed by `Auxilia.Slots.CredentialProbe` (a test-fake slot handler baked into the runner image). `CoreApiDispatch` system suite **3/3 green** (both dummy paths + the credentialed run). Note: the system-test env rebuilds the images from source on a normal run — a stale cached `core-api` image will fail with "no resolution context".
> - **S4 (remaining)** — rate-limit + audit the resolution endpoint (the run-scoped token is already enforced).
