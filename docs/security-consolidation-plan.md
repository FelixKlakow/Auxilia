...
>
> **Delivery status (2026-07-25):** **S1 ✅** (Core.Api resolves + encrypts, run-scoped token, `6f31c2b`), **S2 ✅** (runner resolves via Core and relays ciphertext, `3f486f4`), and the **dispatcher decoupled from the seeded stores** — it takes the run spec (provider types) from the Core command, and the legacy #18 / config-repo dispatch paths are deleted (`7683f22`). All green (solution-wide unit + component). **Greenfield correction (Felix):** no coexistence — the remaining work is a *clean removal*, not a flag. **Remaining (S3):** make `SlotActivationHandler` Core-only (drop the local fallback + the `CoreApiBaseAddress` guard) and **delete the runner's slot-config subsystem outright** — `SlotConfigurationStore`, `WorkflowConfigurationStore`, `SlotConfigurationSeedHandler` + the `slot-configurations` fanout, `SlotConfigurationsSettings`, `DirtyConfigurationDetector`, `ConfigurationResolver`'s credential methods (keep signal-handler resolution), the `SlotConfigurationRecord` / `WorkflowConfigurationRecord` entities — and rewire `WorkflowRegistrationHandler` (keep signal handlers; drop slot-config validation + dirty detection). Then **S4** (rate-limit + audit resolution) and the credentialed **Docker acceptance**.
