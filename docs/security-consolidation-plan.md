...
> - **Docker acceptance** — the deployed runner is now wired to resolve via Core (`CoreApiDispatch` env sets `WorkflowDispatcher:CoreApiBaseAddress` + a `core-api` network alias). The full end-to-end proof still needs a **credentialed dummy workflow** that declares and activates a slot, plus a minimal **slot-handler plugin** baked into the runner + dummy images, so a connector-backed run resolves JIT through the Core on real Docker. (The Core-side resolution and the full runner relay flow are already proven at the component level — `SlotResolutionTests`, `JitSlotActivationComponentTests` — so this is the last integration hop: the runner's real HTTP call to a real Core.Api container.)
> - **S4** — rate-limit + audit the resolution endpoint (the run-scoped token is already enforced).
