...
public sealed class ImplementationPipelineTests
{
    ...
    /// files an agent would) and immediately completes the turn.
    /// </summary>
    private sealed class ScriptedConsole : ISessionHost, IConsoleSessionEventSource
    {
        ...
        public Task StartAsync(TerminalSessionInfo session, CancellationToken ct) => Task.CompletedTask;
        ...
        public async Task SendTextAsync(string text, CancellationToken ct)
        {
            Drives.Add(text);
            OnDrive(text);
            await _handler!(new ConsoleSessionEvent(ConsoleSessionEvent.TurnEnded, "done"), ct);
        }
    ...
    public async Task HappyPath_PlansImplementsReviewsPushesAndClosesTheStory()
    {
        var console = new ScriptedConsole
        {
            OnDrive = prompt =>
            {
                // The scripted "agent": completeness finds nothing open; plan and revisions
        ...
        var git = new FakeGit();
        var author = new DrivenConsoleSession(console, null, console);

        await new ImplementationPipeline(
                workItems, null, null, Context(), author, null, reviewer, git, TimeProvider.System)
            .RunAsync(CancellationToken.None);

        Assert.Multiple(() =>
        {
            Assert.That(console.Drives.Any(d => d.Contains("Assess whether it is COMPLETE", StringComparison.Ordinal)),
                Is.True, "the completeness check drove the console");
    ...
    public void MissingWorkItem_FailsLoudly()
    {
        ...
        var pipeline = new ImplementationPipeline(
            ...
            new DrivenConsoleSession(console, null, console), null, null,
            new FakeGit(), TimeProvider.System);
    ...
    public async Task SkipMode_NeverPushes_AndAnUnboundReviewerSkipsAiReview()
    {
        ...
        var context = ImplementationContext.FromValues(
            "42", _workspace, _output,
            completenessCheck: "false", pushMode: PushModes.Skip);

        await new ImplementationPipeline(
                workItems, null, null, context,
                new DrivenConsoleSession(console, null, console), null, reviewer: null,
