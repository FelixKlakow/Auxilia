...
#   (Scripts/Start-DevStack.ps1, which also statically registers the "implementation" type) ->
...
if (-not (Test-CoreApi)) {
    ...
    & "$repo\Scripts/Start-DevStack.ps1"
    $apiUp = $false
...
if (-not $SkipImageBuild) {
    ...
    docker build -t auxilia-implementation-workflow:system-test `
        -f "$repo\Source\Workflows\Auxilia.Implementation.Workflow\Dockerfile" $repo
...
if ($types.workflowType -notcontains "implementation") {
    Write-Warning "Workflow type 'implementation' is not registered - restart the stack via Scripts/Start-DevStack.ps1 (it seeds the type statically)."
}
...
Write-Host "Turn pacing rides DRIVEN_STUB_DELAY (Start-DevStack sets 4s). Afterwards: Scripts/Cleanup-TestWorkflows.ps1."
