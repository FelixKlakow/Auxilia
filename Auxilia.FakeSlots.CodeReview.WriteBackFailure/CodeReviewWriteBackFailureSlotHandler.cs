...
namespace Auxilia.FakeSlots.CodeReview.WriteBackFailure;

public sealed class CodeReviewWriteBackFailureSlotHandler : ISlotHandler
{
    public void Register(IServiceCollection services, string slotName, Type serviceType, SlotConfiguration configuration)
    {
        switch (slotName)
        {
            ...
            case "pull-request":
                services.AddKeyedScoped<IPullRequestAccess>(slotName, (_, _) => new FakePullRequestAccessWithWriteFailure());
                break;
    ...
    private sealed class FakePullRequestAccessWithWriteFailure : IPullRequestAccess
    {
        ...
        public Task PostCommentAsync(string body, string? filePath = null, int? lineNumber = null, CancellationToken cancellationToken = default)
            => throw new Exception("Simulated write-back failure");
