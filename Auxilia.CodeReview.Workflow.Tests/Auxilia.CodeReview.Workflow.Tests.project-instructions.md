...
Tests for `Source/Auxilia.CodeReview.Workflow` (`PullRequestReviewWorkflow`): whole-workflow scenarios under `Scenarios/` (`Category=Component`) plus uncategorized `[TestFixture]` unit-level fixtures for the individual services/orchestrators.
