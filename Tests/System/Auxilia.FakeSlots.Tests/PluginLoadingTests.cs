...
public class PluginLoadingTests
{
    ...
    private void PublishFakeSlots()
    {
        ...
        {
            var csproj = Path.Combine(_repoRoot, "Tests", "System", project, $"{project}.csproj");
            var publishDir = _tempDir;

            var startInfo = new System.Diagnostics.ProcessStartInfo
            {
                FileName = "dotnet",
                // -nodeReuse:false + UseSharedCompilation=false: persistent MSBuild/Roslyn
                // workers inherit the redirected pipes; with node reuse they outlive the publish
                // and ReadToEnd stalls until their idle timeout (~15 min).
