...
[TestFixture]
[Category("Unit")]
public class WorkflowBuilderLifetimeTests
{
    [Test]
    public void BuildSchema_WithoutWithLifetime_DefaultsToOneShot()
    {
        var builder = (WorkflowBuilder)WorkflowBuilder.Create("test-workflow");

        var schema = builder.BuildSchema();

        Assert.That(schema.Lifetime, Is.EqualTo(WorkflowLifetime.OneShot));
    }

    [Test]
    public void BuildSchema_WithLifetimeLongLiving_CarriesLongLiving()
    {
        var builder = (WorkflowBuilder)WorkflowBuilder.Create("test-workflow")
        ...
        Assert.That(schema.Lifetime, Is.EqualTo(WorkflowLifetime.LongLiving));
    }

    [Test]
    public async Task Run_WithLongLivingLifetime_RegistrationRequestManifestCarriesLongLiving()
    {
        ...
        var builder = (WorkflowBuilder)WorkflowBuilder.Create("long-living-workflow")
            .WithLifetime(WorkflowLifetime.LongLiving);
        ...
        Assert.That(req.Manifest.Lifetime, Is.EqualTo(WorkflowLifetime.LongLiving));
    }

    [Test]
    public async Task Run_LongLivingApplicationWaitingOnDrainSignal_DrainCommandLetsWorkflowExitWithSuccess()
    {
        var bus = new RecordingBus();
        var mockExit = new Mock<IProcessExitService>();
        var context = new FakeWorkflowRunContext(bus, mockExit.Object);
        ...
        var applicationObservedDrain = false;
        var builder = (WorkflowBuilder)WorkflowBuilder.Create("long-living-workflow")
            .WithLifetime(WorkflowLifetime.LongLiving)
            .WithApplication(async (sp, _) =>
            {
                var drainSignal = sp.GetRequiredService<WorkflowDrainSignal>();

                // The drain subscription is registered before the application starts,
                // so delivering the command here signals the injected token.
                bus.DeliverAsync($"workflow-drain-{instanceId}", new DrainWorkflowCommand(instanceId));

                try
                {
                    await Task.Delay(System.Threading.Timeout.Infinite, drainSignal.Token);
                }
                catch (OperationCanceledException)
                {
                    // Drain requested: finish in-flight work and return normally.
                }

                applicationObservedDrain = drainSignal.IsDraining;
            });
        builder._directiveTimeout = TimeSpan.FromSeconds(5);

        await builder.Run([], context);

        Assert.That(applicationObservedDrain, Is.True, "The application must observe the drain signal.");
