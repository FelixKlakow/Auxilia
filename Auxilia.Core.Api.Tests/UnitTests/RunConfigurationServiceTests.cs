...
public sealed class RunConfigurationServiceTests
{
    ...
    {
        var service = NewService(out _);
        await service.CreateAsync(new CreateRunConfiguration("a", "type-a"), CancellationToken.None);
        await service.CreateAsync(new CreateRunConfiguration("b", "type-b"), CancellationToken.None);
